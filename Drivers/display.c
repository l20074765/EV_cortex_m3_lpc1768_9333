...
** Returned value:          none
*********************************************************************************************************/
void led_show(char *str)
{
	unsigned char i,led[4] = {0};
	for(i = 0;i < 4;i++)
	{
		switch(str[i])
		{
			case '0' : 	led[i] = ledTab[LED_O]; break;
			case '1' : 	led[i] = ledTab[LED_1]; break;
			case '2' : 	led[i] = ledTab[LED_2]; break;
			case '3' : 	led[i] = ledTab[LED_3]; break;
			case '4' : 	led[i] = ledTab[LED_4]; break;
			case '5' : 	led[i] = ledTab[LED_5]; break;
			case '6' : 	led[i] = ledTab[LED_6]; break;
			case '7' : 	led[i] = ledTab[LED_7]; break;
			case '8' : 	led[i] = ledTab[LED_8]; break;
			case '9' : 	led[i] = ledTab[LED_9]; break;
			case 'A' : 	led[i] = ledTab[LED_A]; break;
			case 'B' : 	led[i] = ledTab[LED_B]; break;
			case 'C' : 	led[i] = ledTab[LED_C]; break;
			case 'E' : 	led[i] = ledTab[LED_E]; break;
			case 'F' : 	led[i] = ledTab[LED_F]; break;
			case 'H' : 	led[i] = ledTab[LED_H]; break;
			case 'I' : 	led[i] = ledTab[LED_I]; break;
			case 'L' : 	led[i] = ledTab[LED_L]; break;
			case 'P' : 	led[i] = ledTab[LED_P]; break;
			case 'R' : 	led[i] = ledTab[LED_R]; break;
			case 'S' : 	led[i] = ledTab[LED_S]; break;
			case 'U' : 	led[i] = ledTab[LED_U]; break;
			case 'O' : 	led[i] = ledTab[LED_O]; break;
			default: 	led[i] = ledTab[LED_OFF]; break;
		}
	}
...
/*********************************************************************************************************
** Programmer:				
** Function name:           ledPaoMaShow1
** Descriptions:          跑马灯流水显示零0 
** output parameters:       none
** Returned value:          none
*********************************************************************************************************/
...
/*********************************************************************************************************
** Programmer:				
** Function name:           displayMemuLED
** Descriptions:            显示设置菜单
** input parameters:        Memu:要显示菜单号；
**							Memu:0-主菜单；subMenu-二级菜单
** 							如:displayMemuLED(0,0)则显示F0.00
** 							如:displayMemuLED(9,0)则显示F9.00
** 							如:displayMemuLED(0,1)则显示F0.01
** 							如:displayMemuLED(9,1)则显示F9.01
** output parameters:       none
** Returned value:          none
*********************************************************************************************************/
void displayUserMemuLED(unsigned char Memu,unsigned char subMenu)
{
	unsigned char DisBuf[6],temp;
	temp = subMenu % 100;
	DisBuf[0] = ledTab[LED_U];

	if(Memu)
	{
		temp = Memu % 100;
		DisBuf[1] = ledTab[temp/10] ;
		DisBuf[2] = ledTab[temp%10] | 0x80;
		DisBuf[3] = ledTab[subMenu%10];
	}
	else
	{
		temp  = subMenu % 100;
		DisBuf[1] = ledTab[Memu%10] | 0x80;
		DisBuf[2] = ledTab[temp/10] ;
		DisBuf[3] = ledTab[temp%10];
	}
...
/*********************************************************************************************************
** Programmer:				
** Function name:           displayAdminMemuLED
** Descriptions:            显示管理设置菜单
** input parameters:        Memu:要显示菜单号；
**							Memu:0-主菜单；subMenu-二级菜单enterSubMenu 是否进入二级菜单
** 							如:displayMemuLED(0,0)则显示F0.00
** 							如:displayMemuLED(9,0)则显示F9.00
** 							如:displayMemuLED(0,1)则显示F0.01
** 							如:displayMemuLED(9,1)则显示F9.01

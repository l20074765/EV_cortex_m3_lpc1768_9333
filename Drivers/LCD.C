...
#define SCL_SET()   FIO3DIR |= (1ul << 26); FIO3SET |= (1ul << 26)	//SCL Clock,P3.26
#define SCL_CLR()   FIO3DIR |= (1ul << 26); FIO3CLR |= (1ul << 26)
#define SDA_SET()   FIO3DIR |= (1ul << 25); FIO3SET |= (1ul << 25) 	//SDL DATA,P3.25
#define SDA_CLR()   FIO3DIR |= (1ul << 25); FIO3CLR |= (1ul << 25) 
#define nCS_SET()   FIO1DIR |= (1ul << 24); FIO1SET |= (1ul << 24) 	//CS ChipSel,P1.24
#define nCS_CLR()   FIO1DIR |= (1ul << 24); FIO1CLR |= (1ul << 24)
#define CD_SET()    FIO1DIR |= (1ul << 25); FIO1SET |= (1ul << 25) 	//CD Data&CMD,P1.25
#define CD_CLR()    FIO1DIR |= (1ul << 25); FIO1CLR |= (1ul << 25)
...
{
	unsigned int x,y;	
	for(x=0;x<16;x++) 		//写DIS_PAGE页数据
	{
		LCDSetCursor(0,x);
		for(y=0;y<240;y++)		//写DIS_COL列显示数据
		{
			UC1608EPutData(*data++);				//写显示数据0x00
		}

	}
}	
void Pitu_LCDPutPicBmp()
{
...
** input parameters:    mode=1使用哪个logo类型1——pingtu、2——easivend、3——Tuerqi
** output parameters:   无
** Returned value:      无
*********************************************************************************************************/
void LCDPutLOGOBmp(unsigned char x,unsigned char y,unsigned char mode)
{
	...
	{
		case 1:
			LCDPutBmp(x,y,155,40,EasivendLOGO,0);
			break;
		case 2:
